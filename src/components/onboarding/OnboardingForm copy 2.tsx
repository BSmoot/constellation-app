...
import type { QuestionId } from '@/config/questions'
  ...
  async function handleNext() {
    ...
    try {
      if (currentStep < onboardingQuestions.length - 1) {
        ...
        // Process all responses through our API endpoint
        const parsedResponses = await Object.keys(storyData).reduce(
          async (acc, questionId) => {
            try {
              const response = await fetch('/api/parse-response', {
                method: 'POST',
                headers: {
              ...
              console.error(`Error processing ${questionId}:`, error);
              return acc;
            }
          },
          Promise.resolve({})
